...
// => SPI/APB wrapper
extern int tmc4670_readInt(u8 motor, uint8 address);
extern void tmc4670_writeInt(u8 motor, uint8 address, int value);
extern u16 tmc4670_readRegister16BitValue(u8 motor, u8 address, u8 channel);
extern void tmc4670_writeRegister16BitValue(u8 motor, u8 address, u8 channel, u16 value);
// <= SPI/APB wrapper
...
void tmc4670_doEncoderInitializationMode0(u8 motor, u8 *initState, u16 initWaitTime, u16 *actualInitWaitTime, u16 startVoltage)
{
	...
	case STATE_ESTIMATE_OFFSET:
		// todo ADD 2: do offset estimation here #1
...
{
	tmc4670_writeInt(motor, TMC4670_MOTOR_TYPE_N_POLE_PAIRS, 0x000010004);  // select 4 pole pairs and motor type BLDC
	tmc4670_writeInt(motor, TMC4670_PWM_POLARITIES, 1);                     // Low Side 1, and High Side 0
	tmc4670_writeInt(motor, TMC4670_PWM_SV_CHOP, 7);                        // centered PWM for FOC
}

void tmc4670_load_Stepper_Eval_Defaults(u8 motor)
{
	tmc4670_writeInt(motor, TMC4670_MOTOR_TYPE_N_POLE_PAIRS, 50);  // select 50 pole pairs and motor type Stepper
	tmc4670_writeInt(motor, TMC4670_PWM_POLARITIES, 0);            // Low Side 0, and High Side 0
	tmc4670_writeInt(motor, TMC4670_PWM_SV_CHOP, 7);               // centered PWM for FOC
}

void tmc4670_start_Open_Loop_Test(u8 motor)
{
	tmc4670_writeInt(motor, TMC4670_PHI_E_SELECTION, PHI_E_OPEN_LOOP);
	tmc4670_writeRegister16BitValue(motor, TMC4670_MODE_RAMP_MODE_MOTION, BIT_0_TO_15, MOTION_MODE_UQ_UD_EXT);
	tmc4670_writeInt(motor, TMC4670_UQ_UD_EXT, 2000);            // select UQ_EXT=0, UD_EXT=2000
	tmc4670_writeInt(motor, TMC4670_OPENLOOP_ACCELERATION, 60);  // 60 [rpm/s]
	tmc4670_writeInt(motor, TMC4670_OPENLOOP_VELOCITY_TARGET, 5);
}

void tmc4670_load_Hall_Config(u8 motor, u32 mode, u16 PHI_M_Offset, u16 PHI_E_Offset)
{
	tmc4670_writeInt(motor, TMC4670_HALL_MODE, mode);
	tmc4670_writeRegister16BitValue(motor, TMC4670_HALL_PHI_E_PHI_M_OFFSET, BIT_0_TO_15, PHI_M_Offset);
	tmc4670_writeRegister16BitValue(motor, TMC4670_HALL_PHI_E_PHI_M_OFFSET, BIT_16_TO_31, PHI_E_Offset);
	tmc4670_writeRegister16BitValue(motor, TMC4670_MODE_RAMP_MODE_MOTION, BIT_0_TO_15, MOTION_MODE_STOPPED);
	tmc4670_writeInt(motor, TMC4670_PHI_E_SELECTION, PHI_E_HALL);
}

void tmc4670_load_ABN_Config(u8 motor)
{
	tmc4670_writeRegister16BitValue(motor, TMC4670_MODE_RAMP_MODE_MOTION, BIT_0_TO_15, MOTION_MODE_STOPPED);
	tmc4670_writeInt(motor, TMC4670_PHI_E_SELECTION, PHI_E_ABN);
}

void tmc4670_loadDefaultPISettings(u8 motor)
{
	tmc4670_writeInt(motor, TMC4670_PID_FLUX_P_FLUX_I, 0x00FF0200);
	tmc4670_writeInt(motor, TMC4670_PID_TORQUE_P_TORQUE_I, 0x00FF0200);
	tmc4670_writeInt(motor, TMC4670_PID_VELOCITY_P_VELOCITY_I, 0x00FF0200);
	tmc4670_writeInt(motor, TMC4670_PID_POSITION_P_POSITION_I, 0x00200000);
}
